...
A single TPU node (e.g., `tpu-v5e-8`, `tpu-v6e-16`) is often enough for many models. Move to multi-host configurations when:
...
For accelerator configurations spanning more than one node (e.g., `v2-16`, `tpu-v3-32`, `tpu-v5p-16`, `tpu-v6e-2x4`), Kinetic automatically selects the Pathways backend.

```python
import kinetic

# tpu-v5e-2x4 uses two nodes with 4 TPU cores each (8 cores total)
@kinetic.run(accelerator="tpu-v5e-2x4")
...
```python
@kinetic.run(accelerator="tpu-v5e-2x4", backend="pathways")
